...
class Ui_MainWindow(object):
    ...
    def Canny_img(self):  # 边缘检测
        ...
        if not docCnt:
            ...
            # 画出四点区域和角点
            print("Contours with four points found")
            for pts in docCnt:
              cv2.drawContours( Hoffman_img, [pts], 0, (255, 0, 0), 2)  # 绘制轮廓
              for point in pts:
                cv2.circle( Hoffman_img, tuple(point), 10, (0, 0, 255), -1)
        cv2.imshow('signedf',Hoffman_img)
        cv2.imwrite(r"D:\pic\Hoffman.png",Hoffman_img)

        # 绘制角点

        paper = toushi_transform(Hoffman_img, np.array(docCnt[0]))
        warped = toushi_transform(gray, np.array(docCnt[0]))
    ...
    def Positioning_options(self):  # 保存文本文件
        ...
        def judgeX(x, mode):
            if mode == "point":
                if x < 600:
                    ...
                    return int((x - 650) / 100) + 11
                elif 1250 < x < 1900:
                    return int((x - 1250) / 100) + 36
                elif x > 1900:
                    return int((x - 1900) / 100) + 61
        ...
        def judge(x, y, mode):
            if judgeY(y, mode) != False and judgeX(x, mode) != False:
                if mode == "point":
                    return int(y / 560) * 5 + judgeX(x, mode), judgeY(y, mode)
            else:
                return 0
        ...
        for c in cnts1:
            ...
            if (w >= 60) and h >= 45 and 1.2 <= ar <= 2:
                # print(x, y, w, h)
                M = cv2.moments(c)
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
                questionCnts.append(c)
                if (cX < 560 and cY < 1200) or (cX > 560):
